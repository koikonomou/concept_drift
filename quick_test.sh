...
pycheck() {
    ...
    # Prepend sys.path so config/models are importable from project root
    {
        echo "import sys, os"
        echo "sys.path.insert(0, os.path.join(os.getcwd(), 'src'))"
        echo "sys.path.insert(0, 'src')"
        printf '%s\n' "$SCRIPT"
    } > "$TMPPY"
    check "$NAME" python3 "$TMPPY"
}
...
if ! $SKIP_TRAIN; then
    check "Train (3 epochs)" python3 src/train.py --epochs 3
else
    echo ""
...
# ── Step 2: Extract ───────────────────────────────────────────────────────────
check "Extract features" python3 src/extract.py

# ── Step 3: Detect (fast — only 10 MMD permutations) ─────────────────────────
check "Detect drift" python3 src/detect.py --drift-sigma 2.0 --mmd-perms 10
...
check "Fine-tune (Pool A, 25%, 3 epochs)" python3 src/finetune.py \
    --pool data \
    --ft-pcts 25 \
    --ft-epochs 3 \
    --ft-beta 2.0 \
...
check "run_experiment --dry-run" python3 src/run_experiment.py \
    --tag quicktest \
    --dry-run \
    --drift-sigma 2.0 \
    --ft-epochs 3 \
