...
class HASModel(nn.Module):
    ...
    # ADDITION 1 — expose L2-normalised class weight vectors for margin computation
    def get_normed_weights(self):
        """Return L2-normalised HAS weight matrix, shape [64, n_classes].

        Each column is a unit vector pointing to a class prototype on the
        unit sphere.  Used by extract.py to compute per-sample angular margins
        without re-running the full HAS layer.
        """
        return F.normalize(self.has_layer.weight, p=2, dim=0)
